...
class App extends Component {
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        {this.state.user.id ? <div className="quit" onClick={this.signOut.bind(this)}></div> : null}
  ...
  componentDidUpdate() {
    ...
    for (let i = 0; i < li.length; i++) {
      if (todoList[i].status === 'completed') {
        li[i].firstChild.childNodes[1].firstChild.style.display = 'block'
      }
    }
  }
  signOut(){
  ...
  toggle(e, todo){
    ...
    if (todo.status === 'completed') {
      e.target.parentNode.childNodes[1].firstChild.style.display = 'block'
    }else {
      e.target.parentNode.childNodes[1].firstChild.style.display = 'none'
    }
  }
  changeTitle(event){
    this.setState({
